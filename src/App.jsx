          ...
          <div className="grid grid-cols-2 gap-4 pt-2 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {TEAMS.map((team) => {
              ...
              return (
                <div 
                  key={team.id} 
                  className={`glass-card relative flex flex-col h-full border transition-all duration-500 ${
                    isLive ? 'border-brand-orange shadow-[0_0_25px_rgba(255,77,0,0.3)] animate-glow' : 'border-white/5 opacity-80'
                  }`}
                >
            ...
            <div className="space-y-4 pt-4 animate-in fade-in slide-in-from-left-8 duration-500">
                ...
                {TEAMS.map(team => (
                    <div key={team.id} className="glass-card flex items-center justify-between border-white/5 py-4 px-5">
                        ...
                        <span className={`text-[9px] px-2.5 py-1 rounded-full font-black uppercase tracking-wider ${team.id === activeTeamId ? 'bg-red-500/20 text-red-500 border border-red-500/30' : 'bg-white/5 text-white/30'}`}>
                            {team.id === activeTeamId ? 'PITCHING' : 'WAITING'}
                        </span>
                    </div>
                ))}
            </div>
